...
days = ['Day' + str(int(x)) for x in np.arange(1,22.1)] # Several days
mice = ['M' + str(int(x)) for x in np.arange(1,9.1)] # Several mice
#mice = ['M' + str(int(x)) for x in [5]]# specific mice
#days = ['Day' + str(int(x)) for x in [1,7]]# specific day/s
    ...
    for mcount,mouse in enumerate(mice):
        ...
        try:
            ...
            if stopsdata_p.size > 0: # if there are probe trials, plot 3x3 subplots
                ...
                fig.savefig('Plots/Example_Plots/Example' + 'Data' + '_' + str(mouse) + '_' + str(day) + '.png',  dpi = 200)
                plt.close()
